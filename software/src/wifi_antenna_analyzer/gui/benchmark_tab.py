"""Pestaña "Benchmark" de la GUI (Fase 3, + export PDF de la Fase 5).

Componente de UI puro: no sabe nada de puertos serie ni de
BenchmarkSession. Expone callbacks `on_start(duration_seconds)` y
`on_export_pdf()` que la ventana principal (AnalyzerApp) usa para
arrancar la sesión real y para volcar el último resultado a PDF, y
métodos `set_running` / `set_status` / `show_results` para que
...
class BenchmarkTab(ttk.Frame):
    ...
    def _build_controls(self) -> None:
        ...
        self.export_button = ttk.Button(
            bar, text="Exportar PDF", command=self._on_export_pdf, state="disabled"
        )
