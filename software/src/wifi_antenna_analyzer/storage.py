...
from .models import AntennaComparisonRow, BenchmarkResult, NetworkSample
...
class ComparisonCsvWriter:
    """Escribe el resultado de un Comparador de antenas (una fila por red)."""
    ...
    def __init__(self, outdir: Path | str, antenna_a: str, antenna_b: str) -> None:
        self.outdir = Path(outdir)
        self.outdir.mkdir(parents=True, exist_ok=True)

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_a = antenna_a.replace(" ", "_")
        safe_b = antenna_b.replace(" ", "_")
        self.path = self.outdir / f"comparison_{safe_a}_vs_{safe_b}_{timestamp}.csv"

        self._file = self.path.open("w", newline="", encoding="utf-8")
        self._writer = csv.DictWriter(self._file, fieldnames=self.FIELDNAMES)
        self._writer.writeheader()

    def write_rows(self, rows: Iterable[AntennaComparisonRow]) -> None:
        for row in rows:
            self._writer.writerow(
                {
                    "ssid": row.ssid,
                    "mac": row.bssid,
                    "rssi_medio_a": f"{row.mean_a:.2f}" if row.mean_a is not None else "",
                    "rssi_medio_b": f"{row.mean_b:.2f}" if row.mean_b is not None else "",
                    "diferencia_db": f"{row.diff_db:.2f}" if row.diff_db is not None else "",
                }
            )
        self._file.flush()
    ...
    def __enter__(self) -> ComparisonCsvWriter:
        return self
